...
{
  sealed public class Rule_statement:Rule
  {
    private Rule_statement(String spelling, List<Rule> rules) :
      base(spelling, rules)
    {
    }
    ...
    public static Rule_statement Parse(ParserContext context)
    {
      ...
      if (parsed)
      {
        rule = new Rule_statement(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
      }
      else
      {
        context.index = s0;
      ...
      context.Pop("statement", parsed);

      return (Rule_statement)rule;
    }
  }
}
