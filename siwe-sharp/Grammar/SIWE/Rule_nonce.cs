...
{
  sealed public class Rule_nonce:Rule
  {
    private Rule_nonce(String spelling, List<Rule> rules) :
      base(spelling, rules)
    {
    }
    ...
    public static Rule_nonce Parse(ParserContext context)
    {
      context.Push("nonce");
      ...
      if (parsed)
      {
        rule = new Rule_nonce(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
      }
      else
      {
        context.index = s0;
      ...
      context.Pop("nonce", parsed);

      return (Rule_nonce)rule;
    }
  }
}
