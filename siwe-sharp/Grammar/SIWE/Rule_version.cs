...
{
  sealed public class Rule_version:Rule
  {
    private Rule_version(String spelling, List<Rule> rules) :
      base(spelling, rules)
    {
    }
    ...
    public static Rule_version Parse(ParserContext context)
    {
      ...
      if (parsed)
      {
        rule = new Rule_version(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
      }
      else
      {
        context.index = s0;
