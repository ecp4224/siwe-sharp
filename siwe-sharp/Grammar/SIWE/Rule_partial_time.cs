...
{
  sealed public class Rule_partial_time:Rule
  {
    private Rule_partial_time(String spelling, List<Rule> rules) :
      base(spelling, rules)
    {
    }
    ...
    public static Rule_partial_time Parse(ParserContext context)
    {
      context.Push("partial-time");
      ...
      if (parsed)
      {
        rule = new Rule_partial_time(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
      }
      else
      {
        context.index = s0;
      ...
      context.Pop("partial-time", parsed);

      return (Rule_partial_time)rule;
    }
  }
}
