...
namespace SiweSharp.Grammar.SIWE
{
  public class Parser
  {
    ...
    static private Rule Parse(String rulename, String text, bool trace)
    {
      ...
      Rule rule = null;
      if (rulename.ToLower().Equals("ALPHA".ToLower())) rule = Rule_ALPHA.Parse(context);
      ...
      else if (rulename.ToLower().Equals("sign-in-with-ethereum".ToLower())) rule = Rule_sign_in_with_ethereum.Parse(context);
