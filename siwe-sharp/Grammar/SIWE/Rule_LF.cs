...
{
  sealed public class Rule_LF:Rule
  {
    private Rule_LF(String spelling, List<Rule> rules) :
      base(spelling, rules)
    {
    }
    ...
    public static Rule_LF Parse(ParserContext context)
    {
      ...
      if (parsed)
      {
        rule = new Rule_LF(context.text.Substring(a0.start, a0.end - a0.start), a0.rules);
      }
      else
      {
        context.index = s0;
